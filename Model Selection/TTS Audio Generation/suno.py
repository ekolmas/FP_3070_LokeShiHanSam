"""
suno.py — Generate alternating-host TTS segments using Suno Bark

Outputs:
- Segments: TTS Audio Generation/suno_output/seg_000_host1.wav, ...
...
# Bark voice presets (plenty available). Pick two distinct ones:
VOICE_PRESET_1 = "v2/en_speaker_6"
VOICE_PRESET_2 = "v2/en_speaker_9"

MODEL_ID = "suno/bark"
# ------------------------------

BASE_DIR = Path(__file__).resolve().parent
OUT_DIR = BASE_DIR / "suno_output"
...
def main() -> None:
    ...
    for i, text in enumerate(CONVERSATION):
        voice_preset = VOICE_PRESET_1 if (i % 2 == 0) else VOICE_PRESET_2
        host = "host1" if (i % 2 == 0) else "host2"
        seg_path = OUT_DIR / f"seg_{i:03d}_{host}.wav"

        inputs = processor(
            text=text.strip(), voice_preset=voice_preset, return_tensors="pt"
        )
        inputs = {k: v.to(device) for k, v in inputs.items()}

        with torch.no_grad():
            audio = model.generate(**inputs)  # (1, n)
